<template>
  <aside :class="[
    ...
    <div class="flex flex-col h-screen">
      <!-- Logo -->
      <div class="flex items-center justify-between p-4 border-b border-gray-800 shrink-0 logo">
        <router-link to="/" class="flex items-center gap-2">
          <img src="@/assets/logo.svg" alt="ApplyLog Logo" class="logo-img" />
          <span class="text-xl font-bold text-white">ApplyLog</span>
        </router-link>

        <button @click="$emit('update:open', false)" class="lg:hidden text-gray-400 hover:text-white">
          <X class="h-6 w-6" />
        </button>
      </div>
      ...
      <nav class="p-4 space-y-2 flex-1 overflow-y-auto">
        ...
        <div v-if="jobStore.hasPendingChanges"
          ...
          <button @click="handleSync" :disabled="syncStore.isSyncing"
